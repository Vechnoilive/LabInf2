...
public:
    ...
    explicit ArrayMutableSequenceIterator(T* ptr) : ptr_(ptr) {}
...
public:
    ...
    {
        if (data.GetSize() == 0)
            throw std::out_of_range("Попытка получить первый элемент из пустой последовательности");
        return data.Get(0);
    }
    ...
    {
        if (data.GetSize() == 0)
            throw std::out_of_range("Попытка получить последний элемент из пустой последовательности");
    ...
    {
        if (index < 0 || index > data.GetSize())
            throw std::out_of_range("Неверный индекс для вставки");
    ...
    std::unique_ptr<ISequence<T>> GetSubsequence(int startIndex, int endIndex) const override
    {
        if (startIndex < 0 || endIndex >= GetLength() || startIndex > endIndex)
            throw std::out_of_range("Неверные индексы для подпоследовательности");

        const int newSize = endIndex - startIndex + 1;
    ...
    {
        const int minLength = std::min(GetLength(), other->GetLength());
